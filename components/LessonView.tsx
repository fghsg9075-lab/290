...
export const LessonView: React.FC<Props> = ({ 
  ...
  return (
    <div className="bg-white min-h-screen pb-20 animate-in fade-in">
       ...
       {/* Notes Body */}
       <div className="max-w-3xl mx-auto p-6 md:p-10">
           {/* COMPETITIVE THEME OVERRIDE */}
           {(classLevel === 'COMPETITION' || content.subtitle?.includes('Competitive')) ? (
               <div className="prose prose-slate max-w-none">
                   {/* Special Competition Header */}
                   <div className="bg-purple-900 text-white p-6 rounded-xl shadow-lg mb-8 relative overflow-hidden">
                       <div className="relative z-10">
                           <h1 className="text-2xl font-black text-white mb-2">{chapter.title}</h1>
                           <div className="flex items-center gap-2 text-purple-200 text-sm font-bold uppercase tracking-widest">
                               <Trophy size={16} className="text-yellow-400" /> Competitive Exam Notes
                           </div>
                       </div>
                   ...
                   <ReactMarkdown 
                       ...
                       components={{
                           h1: ({node, ...props}) => <h2 className="text-xl font-black mt-8 mb-4 text-purple-900 border-b-2 border-purple-100 pb-2" {...props} />,
                           h2: ({node, ...props}) => <h3 className="text-lg font-bold mt-6 mb-3 text-purple-800 flex items-center gap-2" {...props} />,
                           strong: ({node, ...props}) => <strong className="text-purple-900 font-black bg-purple-50 px-1 rounded" {...props} />,
                           ul: ({node, ...props}) => <ul className="list-none space-y-2 my-4 pl-0" {...props} />,
                           li: ({node, ...props}) => <li className="pl-4 border-l-4 border-purple-200 py-1 bg-slate-50/50 rounded-r-lg" {...props} />,
                           blockquote: ({node, ...props}) => <blockquote className="border-l-4 border-yellow-400 pl-4 py-3 my-6 bg-yellow-50 rounded-r-lg italic text-slate-700 shadow-sm" {...props} />,
                           code: ({node, ...props}) => <code className="bg-slate-800 text-yellow-400 px-1.5 py-0.5 rounded text-sm font-mono font-bold" {...props} />,
                       }}
                   >
                       {content.content}
                   </ReactMarkdown>
