...
const executeWithRotation = async <T>(
    ...
    // 1. Try Custom API Keys (with Rotation & Retry)
    if (keys.length > 0) {
        const shuffledKeys = [...keys].sort(() => 0.5 - Math.random()); // Simple shuffle
        
        for (const key of shuffledKeys) {
            try {
                console.log(`[AI] Attempting with key ending in ...${key.slice(-4)}`);
                ...
                if (isRateLimit) {
                    console.warn(`[AI] Key ...${key.slice(-4)} exhausted. Switching to next key.`);
                    continue; // Try next key
                } else {
                    console.error(`[AI] Non-quota error with key ...${key.slice(-4)}:`, error);
                    // For now, continue to next key even on other errors to be safe, 
                    // or rethrow if it's a prompt error? 
                    // Let's safe failover.
                    continue;
                }
            }
    ...
    // 2. Fallback to Replit AI (If no keys or all failed)
    // Note: Replit AI integration might need specific client. 
    // Assuming user wants robust key rotation primarily.
    console.error("[AI] All available keys failed or no keys found.");
    throw new Error("AI Generation Failed: All API keys exhausted or invalid. Please add working keys in Admin Dashboard.");
};
...
export const fetchLessonContent = async (
  ...
  // Use Override Prompt if available, otherwise fallback to default structure
  let corePrompt = adminPromptOverride || `Write detailed study notes for ${board} Class ${classLevel} ${subject.name}, Chapter: "${chapter.title}".
  Language: ${language}.
  Format: Markdown.
  Structure:
  1. Introduction
