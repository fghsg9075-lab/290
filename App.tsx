...
const App: React.FC = () => {
  ...
  const handleContentGeneration = async (type: ContentType, count?: number, forcePay: boolean = false) => {
    ...
    // RESTRICT STUDENT FROM USING AI GENERATION IF NO CONTENT
    if (state.user.role === 'STUDENT' && !state.originalAdmin) {
       // Only allow consumption of existing content.
       // AI Generation calls below will fail or be blocked if content doesn't exist.
       // We need to check if content exists BEFORE calling fetchLessonContent with allowAiGeneration=true.
       // However, fetchLessonContent handles the 'getAdminContent' check first.
       // We just need to enforce allowAiGeneration = false for students.
    }
    
    const isStudent = state.user.role === 'STUDENT' && !state.originalAdmin;
    ...
    try {
        ...
        const content = await fetchLessonContent(
          ...
          0, false, 15, promptOverride, !isStudent 
        );
